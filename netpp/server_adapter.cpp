...
namespace netpp {
  ...
  bool TCPTransportAdapter::ping(uint64_t sockfd, sockaddr* inaddr) {
    ...
    DWORD transferred;
    DWORD flags = 0;
    BOOL rc = ::WSASend(sockfd, &wbuf, 1, &transferred, flags, NULL, NULL);
    if (rc == SOCKET_ERROR) {
...
#if CLIENT_USE_WSA
    Tag_WSA_BUF buf = { buffer, size, ESocketOperation::E_RECV, nullptr };
    Tag_WSA_OVERLAPPED overlapped = { &buf, ESocketOperation::E_RECV, nullptr };

    DWORD transferred;
    DWORD flags_ = 0;
    BOOL rc = ::WSARecv(sockfd, &buf, 1, &transferred, &flags_, &overlapped, NULL);
    if (rc == SOCKET_ERROR) {
      int error = ::WSAGetLastError();
      if (error != WSA_IO_PENDING) {
        return false;
...
#if CLIENT_USE_WSA
    WSABUF buf = { size, (char*)data };

    DWORD transferred;
    DWORD flags_ = flags ? *flags : 0;
    BOOL rc = ::WSASend(sockfd, &buf, 1, &transferred, flags_, NULL, NULL);
    if (rc == SOCKET_ERROR) {
      int error = ::WSAGetLastError();
      if (error != WSA_IO_PENDING) {
        return false;
