...
bool UNKNOWN_ApplicationAdapter::on_receive(ISocketPipe* pipe, const char* data, uint32_t size, uint32_t flags)
{
    pipe->error(ESocketErrorReason::E_REASON_ADAPTER_UNKNOWN);
    return false;
}

uint32_t UNKNOWN_ApplicationAdapter::calc_size(const char* data, uint32_t size) const
{
    return 0;
}

uint32_t UNKNOWN_ApplicationAdapter::calc_proc_size(const char* data, uint32_t size) const
{
    return 0;
}

bool UNKNOWN_ApplicationAdapter::wants_more_data(const char* data, uint32_t size) const
{
    return false;
}
