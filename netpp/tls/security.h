...
namespace netpp {
  ...
  NETPP_BITWISE_ENUM(ETLSVerifyFlags);

  class TLSSecurityFactory;

  class TLSSecurityController final : public ISecurityController {
  public:
    TLSSecurityController(const TLSSecurityFactory* factory);
    ...
    ESecurityState decrypt(const char* tls_data, uint32_t tls_size, decrypt_cb on_decrypt) override;
  ...
  class TLSSecurityFactory : public ISecurityFactory {
  public:
    TLSSecurityFactory(
      bool is_server,
      ...
      ETLSVerifyFlags verify_flags = ETLSVerifyFlags::VERIFY_PEER
    );
    ~TLSSecurityFactory() override = default;

    ISecurityController* create_controller() override {
      return new TLSSecurityController(this);
    }
    ...
    ETLSVerifyFlags verify_flags() const { return m_verify_flags; }
  ...
  private:
    ...
    ETLSVerifyFlags m_verify_flags;
  };
}
