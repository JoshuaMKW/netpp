...
static int _OPENSSL_print_error(const char* str, size_t len, void* u)
{
    // Print the string exactly as OpenSSL hands it to us
...
bool TLSSecurityController::initialize()
{
    ...
    if (!SSL_CTX_set_cipher_list(m_tls_ctx, "HIGH:!aNULL:!MD5")) {
        ERR_print_errors_cb(_OPENSSL_print_error, this);
...
ESecurityState TLSSecurityController::decrypt(const char* tls_data, uint32_t tls_size, decrypt_cb on_decrypt)
{
    ...
    return on_decrypt(decrypt_out, total_decrypted) ? ESecurityState::E_SUCCEEDED : ESecurityState::E_FAILED;
}
