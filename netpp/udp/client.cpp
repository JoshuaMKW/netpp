...
namespace netpp {
  ...
  UDP_Client::UDP_Client(ISecurityController *security, uint32_t desired_bufsize)
    : m_recv_spec(), m_send_spec(), m_handshake_done(false), m_handshake_state(EAuthState::E_NONE), m_security(security) {
    if (security) {
      assert(security->protocol() == ESecurityProtocol::E_DTLS && "Security controller must be DTLS.");
    }
    ...
    ISocketPipe* pipe = new UDP_Socket(nullptr, &m_recv_allocator, &m_send_allocator, ESocketHint::E_CLIENT);
    if (m_security) {
