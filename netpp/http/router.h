...
namespace netpp {
  ...
  public:
    ...
    void on_get(const std::string& route, get_cb handler) { m_get_cb[route] = handler; }
    void on_head(const std::string& route, head_cb handler) { m_head_cb[route] = handler; }
    void on_post(const std::string& route, post_cb handler) { m_post_cb[route] = handler; }
    void on_put(const std::string& route, put_cb handler) { m_put_cb[route] = handler; }
    void on_delete(const std::string& route, delete_cb handler) { m_delete_cb[route] = handler; }
    void on_options(const std::string& route, options_cb handler) { m_options_cb[route] = handler; }
    void on_trace(const std::string& route, trace_cb handler) { m_trace_cb[route] = handler; }
    void on_connect(const std::string& route, trace_cb handler) { m_connect_cb[route] = handler; }
    void on_patch(const std::string& route, patch_cb handler) { m_patch_cb[route] = handler; }

    void on_unhandled(unhandled_cb handler) { m_unhandled_cb = handler; }

    HTTP_Response* signal_method(const HTTP_Request* request);
